...
std::vector<Zvt::Bmp> Cmd::CompletionForStatus::parse(void)
{
    ...
    bool check = false; // first three bytes are NOT a LLLVAR

    // Is only the status set check must be false
    if (data.size() > 2)
    {
        for (int idx = 0; idx < 3; ++idx)
        {
            // check if this is LLLVar bytes
            if (idx == 0 || (idx == 1 && check) || (idx == 2 && check))
            {
                std::bitset<8> byte(data[idx]);

                check = byte.test(7) && byte.test(6) && byte.test(5) && byte.test(4);
            }

            //cout << "CHECK: " << check << " of " << Utils::log_hex_str(data[idx]) << endl;
        ...
        for (int idx = 0; idx < data.size(); ++idx)
        {
            ...
            switch (idx)
            {
                ...
                case 3:
                    ...
                    if (idx < data.size() - 1)
                    {
                        this->tlv_container.strip_of(LLVar::copyRange(data, ++idx, data.size() - 1));
...
void Cmd::CompletionForStatus::print(void)
{
    ...
    std::cout << "<-PT| version  --> " << version() << endl;
