...
Cmd::BlockReceipt::BlockReceipt() : Zvt::Apdu(0x06, 0xD3), tlv_container(Zvt::Tlv()), receipt_type(0x00) {
}

Cmd::BlockReceipt::~BlockReceipt() {
}

Cmd::BlockReceipt::BlockReceipt(const std::vector<unsigned char> data) : Zvt::Apdu(data.data(), data.size()), tlv_container(Zvt::Tlv()), receipt_type(0x00) {
    this->parse();
}

Cmd::BlockReceipt::BlockReceipt(const BlockReceipt& other) : Zvt::Apdu(other.cmd(), other.size()), tlv_container(other.tlv()), receipt_type(other.type()) {
    this->parse();
}
...
void Cmd::BlockReceipt::print() {
    ...
    write_to_cwd(); //! ### die Zeile sollte raus!
}
