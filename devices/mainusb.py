...
if __name__ == '__main__':
    ...
    usb_manager = UsbSerialManager(vid=0x16C0, pid=0x0483, baudrate=500000, timeout=0.1, required_aliases=teensys)

    async def send_periodically():
        while True:
            # Example commands
            if "RoboCubeFront" in usb_manager.boards:
                await usb_manager.send_move_device_command("RoboCubeFront", "Schleuse", 100, 100, 100)
            if "ServiceCube" in usb_manager.boards:
                await usb_manager.send_move_device_command("ServiceCube", "Lift_A", 100, 100, 100)
            if "RoboCubeBack" in usb_manager.boards:
                await usb_manager.send_move_device_command("RoboCubeBack", "Rodell_C", 10, 100, 100)
